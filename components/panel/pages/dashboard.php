...
<section style="box-shadow: 0px 8px 24px 0px rgba(66, 68, 90, 1);" class="bg-[#3d3d3d] font-[poppins] xs:scale-100 scale-90 text-white rounded-2xl absolute bottom-10 flex items-center justify-center">
        ...
        if(mysqli_num_rows($result) > 0)
             {
                ...
                while($row = mysqli_fetch_assoc($result))
                    {
                        echo '
                            <a onclick="openDetailTab(`todo`,`list='.$row['name'].'`)" id="nav_button_details" class="'.$row['name'].' hover:scale-105 active:scale-95 cursor-pointer hover:shadow-xl hover:text-gray-800 focus:scale-95 px-6 py-3 hover:bg-gray-300 duration-150 ';
                            if($i == 0){
                                $first = $row['name'];
                                echo 'rounded-l-2xl';
                            }
