        ...
        if(mysqli_num_rows($result) > 0)
             {
                ...
                while($row = mysqli_fetch_assoc($result))
                    {
                        echo '
                            <a onclick="moreMenuClose();openDetailTab(`todo`,`list='.$row['name'].'`)" id="nav_button_details" class="'.$row['name'].' hover:scale-105 active:scale-95 cursor-pointer hover:shadow-xl hover:text-gray-800 focus:scale-95 px-6 py-3 hover:bg-gray-300 duration-150 ';
                            if($i == 0){
                                $first = $row['name'];
                                echo 'rounded-l-2xl';
                            }
