...
$sql = "SELECT lists.id, lists.full_name, lists.list_code, concat(users.name, ' ', users.sur_name) as 'full_usr_name', list_invities.id as 'invite_id' FROM lists join users on users.id=lists.owner_id left join list_invities on list_invities.list_id=lists.id WHERE lists.id = $list_id;";
    ...
    <div class="flex flex-col">
        ...
        <div class="mt-6 sm:mt-6 mb-2 flex flex-row items-center gap-2">
            ...
            <button onclick="popupInfoListsDelete()" type="button" class="active:scale-95 duration-150 inline-flex justify-center rounded-2xl px-4 py-2 text-sm font-medium text-rose-400 shadow-sm ring-inset ring-1 ring-rose-400 hover:ring-rose-400 hover:bg-rose-400 hover:text-white hover:shadow-xl duration-150">
                ...
                </svg><?php if($row['invite_id'] != null) { echo '*';} ?>
            </button>

            <?php if($row['invite_id'] != null) { echo '<span class="text-xs text-gray-600">* usuwasz powiązanie listy z Twoim kontem</span>';} ?>
