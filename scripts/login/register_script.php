...
if ($login_sha != "" || $password_sha != "" || $name != "") {
    ...
    if (mysqli_num_rows($result) == 0) {
        $sql = "INSERT INTO users (login, pswd, role_id, status_id, create_date, name, sur_name, mail) VALUES ('$login_sha', '" . $password_sha . "', '3', '1', NOW(), '" . $name . "', '', '" . $login_sha . "')";
        if (mysqli_query($conn, $sql)) {
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                $action_type = '2';
                $desc = 'Dodano listę';
                $sql_log = "INSERT INTO `logs` (`user_id`, `when`, `object_id`, `object_type`, `before`, `after`, `type`, `description`) VALUES ('$owner_id', current_timestamp(), '$object_id', '$object_type', '$before', '$after', '$action_type', '$desc')";
